...
if __name__ == "__main__":
    ...
    KNOWN_ROOT = [WIDTH / 2.0, HEIGHT - 20.0] 
    ...
    for step in range(161):
        ...
        est_rolling, coords_rolling = model.forward_initial_rolling_shutter(DT_SLICE, sigma=SIGMA_LINEWIDTH)
        est_timelapse, coords_timelapse = model.forward_dense_timelapse(T_frames, DT_FRAME, sigma=SIGMA_LINEWIDTH, override_phi_zero=False)
        ...
        # Log Metrics
        if step % 20 == 0:
            print(f"Step {step:03d} | Snapshot Fit: {loss_rolling.item():.5f} | "
                  f"Timelapse Fit: {loss_timelapse.item():.5f} | Avoidance Cost: {loss_avoidance.item():.5f}")
