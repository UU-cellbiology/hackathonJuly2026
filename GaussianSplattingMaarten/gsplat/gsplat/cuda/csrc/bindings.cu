...
std::tuple<
    ...
    torch::Tensor,
    torch::Tensor>
project_complex_gaussians_2d_forward_tensor(
    const int num_points,
    torch::Tensor &means2d,
    torch::Tensor &L_elements,
    const unsigned img_height,
    ...
    project_complex_gaussians_2d_forward_kernel<<<
        (num_points + N_THREADS - 1) / N_THREADS,
        N_THREADS>>>(
        num_points,
        (cuFloatComplex2 *)means2d.contiguous().data_ptr<float>(),
        (cuFloatComplex3 *)L_elements.contiguous().data_ptr<float>(),
        img_size_dim3,
        tile_bounds_dim3,
        clip_thresh,
        // Outputs.
